...
csv_full = os.path.join(OUTPUT_DIR, 'test_metrics_detailed_NCC.csv')
df.to_csv(csv_full, index=False)

numeric = df.drop(columns=['Patient', 'Time_s'])
summary = numeric.agg(['mean', 'std']).T
csv_summary = os.path.join(OUTPUT_DIR, 'test_metrics_summary_NCC.csv')
summary.to_csv(csv_summary, index=True, header=['mean', 'std'])

print("\nDetailed metrics saved to:", csv_full)
print("Summary (mean±std) saved to:", csv_summary)
print("Registered volumes saved in:", VOL_DIR)
print("Deformation fields saved in:", FLOW_DIR)
